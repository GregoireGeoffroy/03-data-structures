...
function calculateAverageHappiness(journal) {
  const totalHappiness = journal.reduce((sum, entry) => sum + entry.happiness, 0);
...
function mostFrequentActivity(journal) {
  ...
  journal.forEach(entry => {
    // Iterate through each event in the entry
    entry.events.forEach(event => {
      // If the activity is already in the count object, increment its count
      activityCounts[event] = (activityCounts[event] || 0) +1;
        activityCounts[event]++;
    });
  });

  // Iterate through the activity count object to find the activity with the highest count
...
function generateJournalReport(journal) {
  ...
  const averageHappiness = calculateAverageHappiness(journal).toFixed(0);
  const mostCommonActivity = mostFrequentActivity(journal);
